...
import me.toaster.mcamusementcore.utils.CompareUtils;
import me.toaster.mcamusementcore.utils.ParseUtils;
import me.toaster.mcamusementcore.utils.UTPSerialize;
...
public class QueueObject extends BukkitRunnable implements UTPSerialize{
	...
	public boolean validateQueue() {
		if(this!=null) {
			//TODO fix legacy sign post.
			if(this.signLocation.getBlock().getType()==Material.WALL_SIGN || 
					this.signLocation.getBlock().getType()==Material.LEGACY_SIGN_POST) {
				if(this.ride!=null) {
					return true;
				}
			}
	...
	public void startScheduler() {
		this.runTaskTimer(MCACore.getProvidingPlugin(MCACore.class), 20L, 20L);
	}
	
	public void updateSign() {
		Location l = this.signLocation;
		Block blk = l.getBlock();
		//fix legacy sign post.
		if(blk.getType()==Material.LEGACY_SIGN_POST || blk.getType()==Material.WALL_SIGN) {
	...
	public File getQueueFile() {
		return new File(MCACore.QUEUE_DIR.getPath()+File.separator+this.rideName+"-queue.utp");
	}
