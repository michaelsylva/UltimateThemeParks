...
import me.toaster.ultimatethemeparks.utils.MessageUtils;

public class UTPBackupCommand extends CommandHandler{

	@Override
	public void commandReceivedPlayer(Player p, Command cmd, String label, String[] args) {
		if(p.hasPermission(UTPPermission.UTP_COMMANDS_BACKUP.getValue())) {
			File dataFolder = UTPCore.UTP_CORE.getDataFolder();
			World w = p.getWorld();
			...
			Bukkit.getScheduler().runTaskAsynchronously(UTPCore.UTP_CORE, new Runnable() {
				@Override
				public void run() {
